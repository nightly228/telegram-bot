...
def show_photos(message):
    ...
    current_photo_index = 0  # Сбрасываем индекс на первый фотографию
...
def send_photo_with_caption(chat_id, index):
    ...
    # Создание клавиатуры для переключения фотографий
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    if index > 0:
...
@bot.message_handler(func=lambda message: message.text in ["Назад", "Вперед"])
